...
{
    internal class OrderBySyntax : SQLSyntax
    {
        ...
        public OrderBySyntax(QueryMetadata metadata, SQLSyntax parentSyntax, Type table, string prop)
            : base(metadata)
        {
        ...
        public override string Generate() =>
            $"{_parentSyntax.Generate()} ORDER BY `{SQLiteHelper.GetTableName(_table)}`.`{_col}`";
    }
}
