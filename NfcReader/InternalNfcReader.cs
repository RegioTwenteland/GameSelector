...
namespace NfcReader
{
    internal class InternalNfcReader
    {
        ...
        private WinSCardWrapper.SCARD_READERSTATE RdrState;
        private string readername;
        private WinSCardWrapper.SCARD_READERSTATE[] states;
        
        private InternalNfcReader()
        {
        }

        private static InternalNfcReader _instance;

        public static InternalNfcReader Instance {
            get
            {
                if (_instance is null)
                {
                    _instance = new InternalNfcReader();
                    _instance.Watch();
                }
        ...
        private void WaitChangeStatus(object sender, DoWorkEventArgs e)
        {
            while (!e.Cancel)
            {
                int nErrCode = WinSCardWrapper.SCardGetStatusChange(hContext, 1000, ref states[0], 1);
        ...
        public WinSCardWrapper.SCARD_IO_REQUEST pioSendRequest;
        private int SendAPDUandDisplay(int reqType)
        {
            ...
            retCode = WinSCardWrapper.SCardTransmit(hCard, ref pioSendRequest, ref SendBuff[0],
                                 SendLen, ref pioSendRequest, ref RecvBuff[0], ref RecvLen);

            if (retCode != WinSCardWrapper.SCARD_S_SUCCESS)
            {
                return retCode;
            }
        ...
        public string GetCardUID()
        {
            ...
            WinSCardWrapper.SCARD_IO_REQUEST request = new WinSCardWrapper.SCARD_IO_REQUEST();
            request.dwProtocol = WinSCardWrapper.SCARD_PROTOCOL_T1;
            request.cbPciLength = System.Runtime.InteropServices.Marshal.SizeOf(typeof(WinSCardWrapper.SCARD_IO_REQUEST));
            byte[] sendBytes = new byte[] { 0xFF, 0xCA, 0x00, 0x00, 0x00 };
            int outBytes = receivedUID.Length;
            int status = WinSCardWrapper.SCardTransmit(hCard, ref request, ref sendBytes[0], sendBytes.Length, ref request, ref receivedUID[0], ref outBytes);

            if (status != WinSCardWrapper.SCARD_S_SUCCESS)
                cardUID = "";
            else
        ...
        public List<string> GetReadersList()
        {
            ...
            //Establish Context
            retCode = WinSCardWrapper.SCardEstablishContext(WinSCardWrapper.SCARD_SCOPE_USER, 0, 0, ref hContext);
            ...
            retCode = WinSCardWrapper.SCardListReaders(this.hContext, null, null, ref pcchReaders);
            ...
            // Fill reader list
            retCode = WinSCardWrapper.SCardListReaders(this.hContext, null, ReadersList, ref pcchReaders);
        ...
        */
        ...
        public bool Connect()
        {
            ...
            connActive = true;
            retCode = WinSCardWrapper.SCardConnect(hContext, readerName, WinSCardWrapper.SCARD_SHARE_SHARED,
                                 WinSCardWrapper.SCARD_PROTOCOL_T0 | WinSCardWrapper.SCARD_PROTOCOL_T1, ref hCard, ref Protocol);
            if (retCode != WinSCardWrapper.SCARD_S_SUCCESS)
            {
                connActive = false;
                return false;
        ...
        public void Disconnect()
        {
            if (connActive)
            {
                retCode = WinSCardWrapper.SCardDisconnect(hCard, WinSCardWrapper.SCARD_UNPOWER_CARD);
            }
        ...
        private void Watch()
        {
            this.RdrState = new WinSCardWrapper.SCARD_READERSTATE();
            readername = GetReadersList()[0];
            this.RdrState.RdrName = readername;

            states = new WinSCardWrapper.SCARD_READERSTATE[1];
            states[0] = new WinSCardWrapper.SCARD_READERSTATE();
            states[0].RdrName = readername;
            states[0].UserData = 0;
            states[0].RdrCurrState = WinSCardWrapper.SCARD_STATE_EMPTY;
